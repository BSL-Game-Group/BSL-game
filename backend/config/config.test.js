...
const DB_ENV_KEYS = ['DB_URL', 'DB_HOST', 'DB_USER', 'DB_PASSWORD', 'DB_NAME', 'DB_PORT', 'DB_NAME_TEST'];
...
test('enables TLS with cert verification when DB_URL requests ssl=true', () => {
  const cfg = loadConfig({
    DB_URL: 'postgresql://u:p@possu-test.it.helsinki.fi:5432/db?targetServerType=primary&ssl=true',
  });
...
test('does NOT force TLS for a DB_URL without ssl=true (local compose)', () => {
  const cfg = loadConfig({ DB_URL: 'postgresql://bsluser:bslpassword@postgres:5432/bsldb' });
  assert.strictEqual(cfg.development.dialectOptions, undefined);
});

test('does NOT force TLS when DB_URL explicitly sets ssl=false', () => {
  const cfg = loadConfig({ DB_URL: 'postgresql://u:p@host:5432/db?ssl=false' });
  assert.strictEqual(cfg.development.dialectOptions, undefined);
});
...
test('seeder tracking is enabled in every environment', () => {
  const cfg = loadConfig({ DB_URL: 'postgresql://u:p@h:5432/db?ssl=true' });
  for (const env of ['development', 'test', 'production']) {
    assert.strictEqual(cfg[env].seederStorage, 'sequelize', `${env} should track seeders`);
  }
});
